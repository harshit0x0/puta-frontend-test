  ...
  return (
    <nav className="bg-blue-800 sticky top-0 z-10 shadow-md flex justify-center">
      <div className="container flex">
        <ul
          className={`flex mx-auto justify-center lg:space-x-4 flex-col md:flex-row`}
        >
          {menuItems.map((item) => (
            <li key={item.name}>
              <Link
                href={item.href}
                className={
                  `block text-center px-4 py-4 text-white hover:bg-blue-700 transition duration-300 ` +
                  (!isDesktop && collapsed ? "hidden" : "") +
                  ` `
                }
          ...
          {!isDesktop && (
            <button
              ...
              className={
                `text-white py-2 mx-auto ` + (collapsed ? "  " : "mx-auto ")
              }
            >
              {" "}
